...
RenderWindow::RenderWindow(Win32System &system, const e00::Vec2<uint16_t> &size)
...
uint8_t RenderWindow::get_pixel(const e00::Vec2<uint16_t> &point) const {
  const auto pos = point.x() + (point.y() * width());
  return _raw_screen.at(pos);
}

void RenderWindow::set_pixel(const e00::Vec2<uint16_t> &point, uint8_t color) {
  const auto pos = point.x() + (point.y() * width());
...
void RenderWindow::blit_bitmap(const e00::Vec2<uint16_t> &to, const e00::resource::Bitmap *source) {
  OutputScreen::blit_bitmap(to, source);
}
