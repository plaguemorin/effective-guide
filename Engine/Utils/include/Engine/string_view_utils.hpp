...
namespace detail {
  ...
  std::error_code stoi_fast_positive_only_2(const std::string_view &sv, T &out) {
    const auto *n = reinterpret_cast<const uint16_t *>(sv.data());
  ...
  std::error_code stoi_fast_positive_only_4(const std::string_view &sv, T &out) {
    const auto *n = reinterpret_cast<const uint32_t *>(sv.data());
...
inline bool string_view_to_bool(const std::string_view& input) {
  if (input.empty()) {
    return false;
  }
