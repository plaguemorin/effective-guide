...
struct formatter<int> {
  ...
  typename std::enable_if<std::is_integral<T>::value, void>::type parse(Context& ctx, T value) {
    char buffer[num_digits(std::numeric_limits<T>::max())];

    if (value < 0) {
      *ctx._out++ = '-';
      parse<Context, T>(ctx, -value);
      return;
    }
...
public:
  std::array<format_arg, num_args> _data;
